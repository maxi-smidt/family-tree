...
export const WorkspaceFileService = {
  async exportDatabase(workspaceId: string, password?: string): Promise<Blob> {
    const response = await api.postRaw(`/workspaces/${workspaceId}/export`, {
  ...
  async exportGedcom(workspaceId: string): Promise<Blob> {
    const response = await api.getRaw(`/workspaces/${workspaceId}/export-gedcom`);
    return response.blob();
  },

  importGedcom(file: File, name?: string): Promise<JobStarted> {
    return api.postForm<JobStarted>(
      "/workspaces/import-gedcom",
