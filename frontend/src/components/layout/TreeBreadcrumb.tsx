...
/**
 * Tree-in-tree breadcrumb. Shows the ancestor chain when the user has followed
...
export const TreeBreadcrumb = () => {
  ...
  const stack = useTreeStore((s) => s.treeNavStack);
  const current = useTreeStore((s) => s.selectedTree);
  const navigateToTreeStack = useTreeStore((s) => s.navigateToTreeStack);
