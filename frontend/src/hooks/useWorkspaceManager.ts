...
export const useWorkspaceManager = () => {
  const deleteTree = useWorkspaceStore((s) => s.deleteTree);
  ...
  const exportDatabase = useCallback(async (tree: Workspace, password?: string) => {
    const blob = await WorkspaceFileService.exportDatabase(tree.id, password);
  ...
  const inspectImport = useCallback(
    async (file: File): Promise<InspectImportResult> =>
      WorkspaceFileService.inspectImport(file),
    [],
  );

  const importDatabase = useCallback(
    async (file: File, password?: string, name?: string) => {
      const { job_id } = await WorkspaceFileService.importDatabase(
        file,
        password,
        name,
      );
  ...
  const exportGedcom = useCallback(async (tree: Workspace) => {
    const blob = await WorkspaceFileService.exportGedcom(tree.id);
    triggerDownload(blob, `${tree.name || "family-tree"}.ged`);
  }, []);

  const importGedcom = useCallback(
    async (file: File, name?: string) => {
      const { job_id } = await WorkspaceFileService.importGedcom(file, name);
      const workspaceId = await useJobStore.getState().trackJob(job_id);
      const tree = await api.get<Workspace>(`/workspaces/${workspaceId}`);
      await loadTrees();
      await selectTree(tree);
