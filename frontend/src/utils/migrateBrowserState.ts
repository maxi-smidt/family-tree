...
/** Rewrites stale v1 entries in `openSheets`: a workspace id the conversion
 *  mapped is rewritten to its current id. A workspace id with no mapping —
 *  either already current or never touched by the conversion — is left
 *  as-is. */
...
export async function migrateV1BrowserState(): Promise<void> {
  ...
  try {
    ...
    const staleIds = Object.keys(openSheets);
