...
- **Subtree extraction now moves a branch into a linked tree** — extracting a subtree no longer creates an unlinked copy that silently diverges; the selected branch is *moved* into a new tree and the selected person stays in both trees as the connecting bridge person (tree-in-tree link). The new default "whole family" selection extracts everyone attached to that person who is not part of the main family — ideal for moving a spouse's side of the family into its own tree — with descendants/ancestors still available. A preview shows members moved, relations severed by the cut, and media size before anything runs. Extraction now requires owning the tree and is gated by the `tree_links` feature flag; linked subtrees survive later deletion of the bridge person in the original tree (#535).
