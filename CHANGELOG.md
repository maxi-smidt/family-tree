...
- **Map view polish** — marker popups now group birthplace/hometown/cemetery/places-lived/event entries under one heading per type (instead of an interleaved list), scroll when a place has many entries, and every member name in them is a clickable link that jumps straight to that person in the tree view (#548). Map tiles dim to match dark mode instead of staying bright white (#549), and are sharper on hi-DPI displays (#556). The map no longer yanks your pan/zoom when changing filters — a "fit to markers" button recenters on demand — the date-range filter now also applies to places-lived entries (not just events), and the filter bar wraps on narrow screens instead of clipping (#551).
