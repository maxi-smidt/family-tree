...
def make_invitation(
    client: TestClient, workspace_id: str, headers: dict, **kwargs
) -> dict:
    payload = {"role": "editor", **kwargs}
    r = client.post(
        f"{API}/workspaces/{workspace_id}/invitations", json=payload, headers=headers
    )
...
def test_owner_can_list_invitations(client, db):
    ...
    r = client.get(f"{API}/workspaces/{tree.id}/invitations", headers=auth(alice))
...
def test_accept_invitation_creates_membership(client, db):
    ...
    r = client.get(f"{API}/workspaces/{tree.id}/access", headers=auth(bob))
    assert r.status_code == 200
...
def test_invite_preview_does_not_require_auth(client, db):
    ...
    assert r.status_code == 200
    body = r.json()
    assert body["workspace_name"] == tree.name
