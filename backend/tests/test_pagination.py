...
def test_members_preserve_default_full_list_and_support_limit_offset(client, db):
    ...
    default = client.get(f"{API}/workspaces/{tree.id}/members", headers=auth(user))
...
def test_family_collection_endpoints_support_pagination(client, db):
    ...
    diseases = client.get(
        f"{API}/workspaces/{tree.id}/diseases",
        headers=auth(user),
        params={"limit": 1, "offset": 1},
    )
...
def test_content_collection_endpoints_support_pagination(client, db):
    ...
    db.add_all(
        [
            GalleryImage(
                id="g0", workspace_id=tree.id, title="A", uploaded_at="2024-01-01"
            ),
            GalleryImage(
                id="g1", workspace_id=tree.id, title="B", uploaded_at="2024-01-02"
            ),
    ...
    assert _ids(
        client.get(
            f"{API}/workspaces/{tree.id}/gallery/images", headers=headers, params=params
        )
    ) == ["g1"]
    assert client.get(
        f"{API}/workspaces/{tree.id}/gallery/links", headers=headers, params=params
    ).json() == [
        {
            "gallery_image_id": "g1",
            "member_id": "m1",
    ...
    assert _ids(
        client.get(f"{API}/workspaces/{tree.id}/events", headers=headers, params=params)
    ) == ["e1"]
    assert client.get(
        f"{API}/workspaces/{tree.id}/events/links", headers=headers, params=params
    ).json() == [{"event_id": "e1", "member_id": "m1"}]
    assert _ids(
        client.get(f"{API}/workspaces/{tree.id}/stories", headers=headers, params=params)
    ) == ["s1"]
    assert client.get(
        f"{API}/workspaces/{tree.id}/stories/links", headers=headers, params=params
