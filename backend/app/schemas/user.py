...
class StoredUserPreferences(BaseModel):
    ...
    Multiplexes unrelated concerns (tutorial progress, image storage choice)
    into one column; this model is the single source of truth for what's
    actually persisted there, read and written through
    ``model_validate``/``model_dump`` instead of raw ``dict`` access.
    ``tab_preferences`` is a separate, genuinely free-form per-user layout
    column and is intentionally not covered here.
    """
