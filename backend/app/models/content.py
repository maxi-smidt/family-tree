...
class GalleryUnknownFace(Base):
    ...
    No ``workspace_id`` column: like ``GalleryMemberLink``, the tree is reached
...
class DocumentFile(Base):
    ...
    The bytes live on disk under ``DATA_PATH/media/<workspace_id>/`` and ``url`` is
    ...
    __table_args__ = (Index("ix_document_files_workspace_id_url", "workspace_id", "url"),)
