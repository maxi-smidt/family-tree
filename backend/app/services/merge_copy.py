...
@dataclass
class MergeContext:
    """Id maps and source/target trees threaded through the copy phase.
        ...
        for img in db.scalars(select(GalleryImage).where(GalleryImage.tree_id == t.id)):
            ...
            db.add(
                GalleryImage(
                    id=new_id,
                    tree_id=ctx.new_tree_id,
                    image_data=copy_media_to_tree(img.image_data, ctx.new_tree_id),
                    title=img.title,
...
def copy_documents(db: Session, ctx: MergeContext) -> None:
    ...
    for t in ctx.sources:
        for f in db.scalars(
            ...
            new_url = f.url
            if f.kind == "file":
                new_url = copy_media_to_tree(f.url, ctx.new_tree_id) or f.url
            db.add(
                DocumentFile(
                    id=str(uuid4()),
                    tree_id=ctx.new_tree_id,
