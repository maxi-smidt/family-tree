...
def _tree_model_bytes_for_tree_ids(db: Session, workspace_ids: list[str]) -> int:
    """Sum structured-row bytes for the supplied tree IDs in batched queries.

    Covers every model that carries ``workspace_id``:
    ...
    def _sum_model(model_cls, filter_col):
        rows = db.scalars(sa.select(model_cls).where(filter_col.in_(workspace_ids))).all()
        return sum(_row_bytes(r) for r in rows)

    # Core genealogy
    total += _sum_model(Member, Member.workspace_id)
    total += _sum_model(Relation, Relation.workspace_id)
    total += _sum_model(MemberDisease, MemberDisease.workspace_id)
    total += _sum_model(MemberTask, MemberTask.workspace_id)
    # MemberTaskLink has no workspace_id column — traverse via MemberTask
    task_links = (
        db.execute(
            sa.select(MemberTaskLink)
            .join(MemberTask, MemberTask.id == MemberTaskLink.task_id)
            .where(MemberTask.workspace_id.in_(workspace_ids))
        )
        .scalars()
        .all()
    )
    ...
    # Gallery
    total += _sum_model(GalleryImage, GalleryImage.workspace_id)
    # GalleryMemberLink has no workspace_id column — traverse via GalleryImage
    links = (
        db.execute(
            sa.select(GalleryMemberLink)
            .join(GalleryImage, GalleryImage.id == GalleryMemberLink.gallery_image_id)
            .where(GalleryImage.workspace_id.in_(workspace_ids))
        )
        .scalars()
        .all()
    )
    total += sum(_row_bytes(r) for r in links)
    # GalleryUnknownFace has no workspace_id column either — traverse via GalleryImage
    unknown_faces = (
        db.execute(
            sa.select(GalleryUnknownFace)
    ...
    # Events
    total += _sum_model(Event, Event.workspace_id)
    ev_links = (
        db.execute(
            sa.select(EventMemberLink)
            .join(Event, Event.id == EventMemberLink.event_id)
            .where(Event.workspace_id.in_(workspace_ids))
        )
        .scalars()
        .all()
    )
    ...
    # Stories
    total += _sum_model(Story, Story.workspace_id)
    story_links = (
        db.execute(
            sa.select(StoryMemberLink)
            .join(Story, Story.id == StoryMemberLink.story_id)
            .where(Story.workspace_id.in_(workspace_ids))
        )
        .scalars()
        .all()
    )
    ...
    # Documents
    total += _sum_model(Document, Document.workspace_id)
    total += _sum_model(DocumentFile, DocumentFile.workspace_id)
    # The three document link tables have no workspace_id column — traverse via
    # Document, which always carries one.
    doc_member_links = (
        db.execute(
            sa.select(DocumentMemberLink)
...
def _tree_model_bytes(db: Session, workspace_id: str) -> int:
    """Sum the estimated byte footprint of all structured rows for *workspace_id*."""
    return _tree_model_bytes_for_tree_ids(db, [workspace_id])


def _media_bytes(workspace_id: str) -> int:
    """Sum on-disk file sizes under ``media_root/<workspace_id>/``.
...
def compute_usage(db: Session, workspace_id: str) -> UsageBreakdown:
    """Return ``{tree_bytes, media_bytes, total_bytes}`` for *workspace_id*."""
    tb = _tree_model_bytes(db, workspace_id)
    mb = _media_bytes(workspace_id)
...
def compute_owner_usage(db: Session, owner_id: str) -> UsageBreakdown:
    ...
    """
    workspace_ids = list(
        db.scalars(sa.select(Workspace.id).where(Workspace.owner_id == owner_id))
    )
    tb = _tree_model_bytes_for_tree_ids(db, workspace_ids)
    mb = sum(_media_bytes(workspace_id) for workspace_id in workspace_ids)
    return {"tree_bytes": tb, "media_bytes": mb, "total_bytes": tb + mb}
...
def check_workspace_quota(db: Session, tree, incoming_bytes: int) -> None:
    ...
    usage = compute_owner_usage(db, tree.owner_id)
    _check_bucket("tree", quotas["tree_quota_bytes"], usage["tree_bytes"], incoming_bytes)
...
def media_warning(db: Session, tree) -> MediaQuotaWarning | None:
    ...
    if used >= quota * _WARNING_THRESHOLD:
        return {"workspace_id": tree.id, "used_bytes": used, "quota_bytes": quota}
    return None
...
def check_full_usage_quota(db: Session, tree) -> None:
    ...
    Unlike check_workspace_quota/check_media_quota (which project an additional
