...
def _is_accessible(db: Session, user: User, t: Tree) -> bool:
    return (
        user.is_admin
...
def compute_link_graph(db: Session, tree: Tree, user: User) -> LinkGraphOut:
    """Graph of trees reachable from ``tree`` via tree-in-tree member links.

    BFS over ``member.linked_tree_id`` starting at ``tree``. Trees ``user``
