...
from app.services.media.storage import copy_media_to_workspace, delete_media
...
def split_linked_entities(
    ...
    for entity_id, entity_links in by_entity.items():
        if all(lnk.member_id in moved for lnk in entity_links):
            ...
            stale_links.extend(lnk for lnk in entity_links if lnk.member_id in moved)
    return moved_ids, stale_links


def load_member_links(db: Session, workspace_id: str) -> tuple[list, list, list]:
    """All gallery/event/story member links of a tree (link tables carry no
    workspace_id, so they are reached through their tree-scoped entity)."""
    gallery_links = list(
        db.scalars(
    ...
    story_links = list(
        db.scalars(
            ...
            .where(Story.workspace_id == workspace_id)
        )
    )
...
def linked_document_ids(
    ...
    *,
    ...
    """Document ids (of ``source_workspace_id``) linked to any moving entity.
    ...
    if moved_member_ids:
        doc_ids |= set(
            db.scalars(
                ...
                .where(
                    Document.workspace_id == source_workspace_id,
...
def copy_documents_for_move(
    ...
    *,
    ...
    — with its files (``copy_media_to_workspace`` for ``kind == "file"``) — into the
    ...
    for old_doc_id in doc_ids:
        ...
        for f in db.scalars(
            ...
            new_url = f.url
            if f.kind == "file":
                new_url = copy_media_to_workspace(f.url, new_tree.id) or f.url
            db.add(
                DocumentFile(
                    id=str(uuid4()),
                    workspace_id=new_tree.id,
    ...
    member_links = (
        list(
            db.scalars(
                ...
                .join(Document, Document.id == DocumentMemberLink.document_id)
                .where(
                    Document.workspace_id == source_workspace.id,
                    DocumentMemberLink.member_id.in_(moved_member_ids),
                )
            )
        )
    ...
    _gc_orphaned_documents(db, source_workspace.id, set(doc_copy_map))


def _gc_orphaned_documents(
    db: Session, source_workspace_id: str, doc_ids: set[str]
) -> None:
    ...
    for doc_id in doc_ids:
        ...
        if doc is None or doc.workspace_id != source_workspace_id:
            continue
        has_link = any(
            db.scalar(
