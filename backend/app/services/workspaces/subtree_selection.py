"""Branch selection for section previews.

Given a root member and a ``direction``, decides which other members belong
in that branch, and classifies a tree's relations against that selection.
Pure over already-loaded rows — no writes, no session commits — so it is
usable from ``app.services.sections``' preview computation.
"""
...
def _load_relations(db: Session, workspace_id: str) -> list[Relation]:
    return list(db.scalars(select(Relation).where(Relation.workspace_id == workspace_id)))
...
def _collect_direct_family_ids(db: Session, workspace_id: str, root_id: str) -> set[str]:
    ...
    """
    relations = _load_relations(db, workspace_id)
...
def _collect_partnership_ids(db: Session, workspace_id: str, root_id: str) -> set[str]:
    ...
    Deliberately simple: in tangled workspaces (e.g. two siblings married into
...
def collect_member_ids(
    db: Session, workspace_id: str, root_id: str, direction: str
) -> set[str]:
    """Return the set of member ids that belong in the sub-tree for ``direction``."""
    root = db.scalar(
        select(Member).where(Member.workspace_id == workspace_id, Member.id == root_id)
    )
    if root is None:
        raise NotFoundError("Root member not found in tree")

    if direction == "partnership":
        return _collect_partnership_ids(db, workspace_id, root_id)
    return _collect_direct_family_ids(db, workspace_id, root_id)
