...
role, ownership, and per-tree content restrictions. Shared restriction
definitions live in `backend/app/services/trees/restrictions.py` and
`frontend/src/lib/contentRestrictions.ts`; do not reintroduce runtime feature
flags without a separate architectural decision.
